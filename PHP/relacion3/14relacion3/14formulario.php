...
<head>
    ...
    <title>Relación 3 - Ejercicio 14 - Formulario</title>
...
<body>
    <section class="container border border-warning rounded shadow mt-5 mb-3 p-5 col-4">
        <form id="form1" action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
            <h3 class="text-center  text-info-emphasis pb-3">Funciones anónimas</h3>
        ...
        <div class="text-center my-5">
            <?php
                if (isset($_POST["texto"]))
                {
                    ...
                    {
                        return round(2 * M_PI * $n, 2);
                    };

                    /* Calcular el área de un círculo */
                    $circulo = function ($n) 
                    {
                        return round(M_PI * pow($n, 2), 2);
                    };

                    /* Calcular el volumen de una esfera */
                    $esfera = function ($n) 
                    {
                        return round((4 * M_PI * pow($n, 3)) / 3, 2);
                    };
